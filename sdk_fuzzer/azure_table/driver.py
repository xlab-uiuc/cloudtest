...
def extract_discrepancy(file):
    ...
    # write to file
    with open('logs/azure_table/unique_discrepancy.txt', 'a') as f:
        f.write('\n\n'.join(li))
            ...
            try:
                ...
                if not arg[t_count] == None:
                    ...
                    if result[0]:
                        ...
                        with open('logs/azure_table/discrepancy.txt', 'a') as f:
...
def run_sequences(methods, type, runs):
    ...
    with io.StringIO() as buf, redirect_stdout(buf):
        ...
        with open('logs/azure_table/discrepant_sequences.txt', 'a') as f:
            f.write("\n".join(discrepant_seqs))  

        output = buf.getvalue().strip()

    # write buf to a new file
    with open('logs/azure_table/log_all.txt', 'a') as f:
        f.write(output)
...
def run_sequences_permutation(methods):
    ...
    t_count = 0
    discrepant_seq = []
    methods_list = methods['tc'] + methods['tsc'] 
...
def run1v1(arg, methods, t_count):
    ...
    f = open("logs/azure_table/discrepant_methods.txt", "r")
    ...
    with io.StringIO() as buf, redirect_stdout(buf):
        ...
        d_count = run_ops(arg["1"], methods['tc'], "tc", d_count, discrepant_methods)
        # run table service client ops
        d_count = run_ops(arg["2"], methods['tsc'], "tsc", d_count, discrepant_methods)
    ...
    # store methods names in order to skip in the next run
    with open('logs/azure_table/discrepant_methods.txt', 'a') as f:
...
def main(arg):
    ...
    total_methods = methods['tc'] + methods['tsc']
    ...
    if arg == ():
        arg = {}
        arg['1'] = [[]] * len(methods['tc'])
        arg['2'] = [[]] * len(methods['tsc'])
    else:
        assert len(arg) == 2, "Invalid number of clients"
        assert len(arg['1']) == len(methods['tc']), "Invalid number of methods for TableClient"
        assert len(arg['2']) == len(methods['tsc']), "Invalid number of methods for TableServiceClient"
